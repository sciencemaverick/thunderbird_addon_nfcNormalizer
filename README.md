...
macOS에서 Thunderbird로 메일을 작성할 때 첨부 파일명이 NFD(자소 분리형)로 들어오면, 원본 파일은 건드리지 않고 compose 창의 첨부 표시 이름만 Unicode NFC로 보정하는 MailExtension 실험 프로젝트입니다.
...
현재까지 유효한 최신 테스트에서는, add-on이 로드된 상태에서 실제 NFD 파일 3개와 NFC 파일 1개를 첨부해 자기 자신에게 보낸 뒤 `Downloads`에 저장한 결과가 모두 기대대로 확인되었습니다.
...
따라서 현재 구현은 최소한 `Thunderbird compose -> self-send -> macOS Downloads 저장` 경로에서는 목적을 달성한 것으로 봅니다.
